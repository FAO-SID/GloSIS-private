-- project
UPDATE spatial_metadata.project SET project_name='Global Salt-Affected Soils' WHERE country_id='MN' AND project_id='GSAS';
-- UPDATE spatial_metadata.project SET project_name='Global Soil Nutrients Map' WHERE country_id='MN' AND project_id='GSNM';
-- UPDATE spatial_metadata.project SET project_name='Global Soil Organic Carbon Sequestration potential map' WHERE country_id='MN' AND project_id='GSOCSEQ';


-- dates
UPDATE spatial_metadata.mapset SET creation_date = '2021-11-16', publication_date = '2021-11-16', revision_date = '2021-11-16' WHERE country_id = 'MN' AND project_id = 'GSAS';
-- UPDATE spatial_metadata.mapset SET creation_date = '2024-12-31', publication_date = '2024-12-31', revision_date = '2024-12-31' WHERE country_id = 'MN' AND project_id = 'GSNM';
-- UPDATE spatial_metadata.mapset SET creation_date = '2022-04-13', publication_date = '2022-04-13', revision_date = '2022-04-13' WHERE country_id = 'MN' AND project_id = 'GSOCSEQ';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    ...
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'MN' AND project_id='GSAS';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'MN' AND project_id='GSNM';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'MN' AND project_id='GSOCSEQ';
...
WHERE m.mapset_id = l.mapset_id AND m.country_id = 'MN';


-- keywords
UPDATE spatial_metadata.mapset SET keyword_place = '{Mongolia}'::text[] WHERE country_id = 'MN';
UPDATE spatial_metadata.mapset m SET keyword_theme = p.keyword_theme FROM spatial_metadata.property p WHERE country_id = 'MN' AND m.property_id = p.property_id;


-- licence
UPDATE spatial_metadata.mapset SET other_constraints = 'CC-BY-4.0' WHERE country_id = 'MN';


-- time period
UPDATE spatial_metadata.mapset SET time_period_begin = '2008-01-01' WHERE country_id = 'MN';
UPDATE spatial_metadata.mapset SET time_period_end = '2019-12-31' WHERE country_id = 'MN';
...
INSERT INTO spatial_metadata.organisation (organisation_id, url, email, country, city, postal_code, delivery_point) VALUES
    ('Mongolian University of Life Sciences - Soil Science Centre', 'http://www.ipas.edu.mn', NULL, 'Mongolia', 'Darkhan', '45047', 'Bag 15')
    ON CONFLICT (organisation_id) DO NOTHING;


-- insert individual
INSERT INTO spatial_metadata.individual (individual_id, email) VALUES
    ('Munkhtsetseg Dorj', 'munkhtststg.do@gmail.com'),
    ('Enkhtuya Bazarradnaa', 'bazarradnaaenkhtuya@gmail.com'),
    ('Kostiantyn Viatkin', NULL)
...
INSERT INTO spatial_metadata.proj_x_org_x_ind (country_id, project_id, tag, "role", "position", organisation_id, individual_id)
    SELECT DISTINCT country_id, project_id, 'contact', 'resourceProvider', 'Officer', 'Mongolian University of Life Sciences - Soil Science Centre', 'Munkhtsetseg Dorj' FROM spatial_metadata.project WHERE country_id = 'MN'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Officer', 'Mongolian University of Life Sciences - Soil Science Centre', 'Munkhtsetseg Dorj' FROM spatial_metadata.project WHERE country_id = 'MN'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Director', 'Mongolian University of Life Sciences - Soil Science Centre', 'Enkhtuya Bazarradnaa' FROM spatial_metadata.project WHERE country_id = 'MN'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Officer', 'Mongolian University of Life Sciences - Soil Science Centre', 'Kostiantyn Viatkin' FROM spatial_metadata.project WHERE country_id = 'MN'
...
INSERT INTO spatial_metadata.url (mapset_id, protocol, url, url_name)
    ...
    --     UNION
    SELECT DISTINCT mapset_id, 'WWW:LINK-1.0-http--link', 'https://www.fao.org/global-soil-partnership/resources/highlights/detail/en/c/1471478/', 'Project webpage' FROM spatial_metadata.mapset WHERE project_id = 'GSOCSEQ' AND country_id = 'MN'
      UNION
    ...
    SELECT m.mapset_id, 'WWW:LINK-1.0-http--link', 'https://storage.googleapis.com/fao-gismgr-glosis-data/DATA/GLOSIS/MAPSET/'||l.mapset_id||'/GLOSIS.'||l.mapset_id||'.D-'||l.dimension_depth||'.D-'||l.dimension_stats||'.'||l.file_extension , 'Download '||l.dimension_depth||' '||l.dimension_stats
        ...
        WHERE m.country_id = 'MN'
          ...
          AND l.dimension_stats IS NOT NULL
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'https://data.apps.fao.org/map/wmts/wmts?service=WMTS&amp;request=GetCapabilities&amp;version=1.0.0&amp;workspace=GLOSIS', 'WMTS (FAO)' FROM spatial_metadata.mapset WHERE country_id = 'MN'
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'http://localhost:8082/?map=/etc/mapserver/'||layer_id||'.map&amp;SERVICE=WMS&amp;REQUEST=GetCapabilities', 'WMTS (Mongolia SIS)' FROM spatial_metadata.layer WHERE layer_id LIKE 'MN-%'
...
('MN-GSAS-SALT-2021', -9999, '-9999', 'No Data', '#000000', 0, 't'),
('MN-GSAS-SALT-2021', 2, '2', '2 - Slight Salinity', '#90ee90', 1, 't'),
('MN-GSAS-SALT-2021', 3, '3', '3 - Slight Sodicity', '#add8e6', 1, 't')
...
BEGIN
    FOR f IN
        SELECT mapset_id FROM spatial_metadata.layer WHERE mapset_id ILIKE 'MN%' AND stats_minimum < -100
    LOOP
