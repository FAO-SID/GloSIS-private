-- project
-- UPDATE spatial_metadata.project SET project_name='Global Salt-Affected Soils' WHERE country_id='ID' AND project_id='GSAS';
-- UPDATE spatial_metadata.project SET project_name='Global Soil Nutrients Map' WHERE country_id='ID' AND project_id='GSNM';
UPDATE spatial_metadata.project SET project_name='Global Soil Organic Carbon Sequestration potential map' WHERE country_id='ID' AND project_id='GSOCSEQ';


-- dates
-- UPDATE spatial_metadata.mapset SET creation_date = '2021-11-13', publication_date = '2021-11-13', revision_date = '2021-11-13' WHERE country_id = 'ID' AND project_id = 'GSAS';
-- UPDATE spatial_metadata.mapset SET creation_date = '2024-12-31', publication_date = '2024-12-31', revision_date = '2024-12-31' WHERE country_id = 'ID' AND project_id = 'GSNM';
UPDATE spatial_metadata.mapset SET creation_date = '2021-08-25', publication_date = '2021-08-25', revision_date = '2021-08-25' WHERE country_id = 'ID' AND project_id = 'GSOCSEQ';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    ...
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'ID' AND project_id='GSAS';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'ID' AND project_id='GSNM';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'ID' AND project_id='GSOCSEQ';
...
WHERE m.mapset_id = l.mapset_id AND m.country_id = 'ID';


-- keywords
UPDATE spatial_metadata.mapset SET keyword_place = '{Indonesia}'::text[] WHERE country_id = 'ID';
UPDATE spatial_metadata.mapset m SET keyword_theme = p.keyword_theme FROM spatial_metadata.property p WHERE country_id = 'ID' AND m.property_id = p.property_id;


-- licence
UPDATE spatial_metadata.mapset SET other_constraints = 'CC-BY-NC-4.0' WHERE country_id = 'ID';


-- time period
UPDATE spatial_metadata.mapset SET time_period_begin = '1970-01-01' WHERE country_id = 'ID';
UPDATE spatial_metadata.mapset SET time_period_end = '2021-08-25' WHERE country_id = 'ID';
...
INSERT INTO spatial_metadata.organisation (organisation_id, url, email, country, city, postal_code, delivery_point) VALUES
    ('BRMP SDLP', 'https://sdlahan.brmp.pertanian.go.id/', 'brmp.sdlahan@pertanian.go.id', 'Indonesia', 'Bogor', '16124', 'Jalan Tentara Pelajar No. 12')
...
INSERT INTO spatial_metadata.proj_x_org_x_ind (country_id, project_id, tag, "role", "position", organisation_id, individual_id)
    SELECT DISTINCT country_id, project_id, 'contact', 'resourceProvider', 'Standardization Analyst', 'BRMP SDLP', 'Adib Hasanawi' FROM spatial_metadata.project WHERE country_id = 'ID'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Standardization Analyst', 'BRMP SDLP', 'Adib Hasanawi' FROM spatial_metadata.project WHERE country_id = 'ID'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Standardization Analyst', 'BRMP SDLP', 'Setiyo Purwanto' FROM spatial_metadata.project WHERE country_id = 'ID'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Standardization Analyst', 'BRMP SDLP', 'Fransiscus Benhardi Wastuwidya' FROM spatial_metadata.project WHERE country_id = 'ID'
...
INSERT INTO spatial_metadata.url (mapset_id, protocol, url, url_name)
    ...
    --     UNION
    SELECT DISTINCT mapset_id, 'WWW:LINK-1.0-http--link', 'https://www.fao.org/global-soil-partnership/resources/highlights/detail/en/c/1471478/', 'Project webpage' FROM spatial_metadata.mapset WHERE project_id = 'GSOCSEQ' AND country_id = 'ID'
      UNION
    ...
    SELECT m.mapset_id, 'WWW:LINK-1.0-http--link', 'https://storage.googleapis.com/fao-gismgr-glosis-data/DATA/GLOSIS/MAPSET/'||l.mapset_id||'/GLOSIS.'||l.mapset_id||'.D-'||l.dimension_depth||'.D-'||l.dimension_stats||'.'||l.file_extension , 'Download '||l.dimension_depth||' '||l.dimension_stats
        ...
        WHERE m.country_id = 'ID'
          ...
          AND l.dimension_stats IS NOT NULL
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'https://data.apps.fao.org/map/wmts/wmts?service=WMTS&amp;request=GetCapabilities&amp;version=1.0.0&amp;workspace=GLOSIS', 'WMTS (FAO)' FROM spatial_metadata.mapset WHERE country_id = 'ID'
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'http://localhost:8082/?map=/etc/mapserver/'||layer_id||'.map&amp;SERVICE=WMS&amp;REQUEST=GetCapabilities', 'WMTS (Indonesia SIS)' FROM spatial_metadata.layer WHERE layer_id LIKE 'ID-%'
...
-- ('ID-GSAS-SALT-2021', -9999, '-9999', 'No Data', '#000000', 0, 't'),
-- ('ID-GSAS-SALT-2021', 1, '1', '1 - Extreme Salinity', '#ff0000', 1, 't'),
-- ('ID-GSAS-SALT-2021', 2, '2', '2 - Moderate Salinity', '#f5deb3', 1, 't'),
-- ('ID-GSAS-SALT-2021', 3, '3', '3 - Moderate Sodicity', '#ee82ee', 1, 't'),
-- ('ID-GSAS-SALT-2021', 4, '4', '4 - None', '#ffffff', 1, 't'),
-- ('ID-GSAS-SALT-2021', 5, '5', '5 - Saline Sodic', '#00ffff', 1, 't'),
-- ('ID-GSAS-SALT-2021', 6, '6', '6 - Slight Salinity', '#90ee90', 1, 't'),
-- ('ID-GSAS-SALT-2021', 7, '7', '7 - Slight Sodicity', '#add8e6', 1, 't'),
-- ('ID-GSAS-SALT-2021', 8, '8', '8 - Strong Salinity', '#f08080', 1, 't'),
-- ('ID-GSAS-SALT-2021', 9, '9', '9 - Strong Sodicity', '#da70d6', 1, 't'),
-- ('ID-GSAS-SALT-2021', 10, '10', '10 - Very Strong Salinity', '#f84040', 1, 't'),
-- ('ID-GSAS-SALT-2021', 11, '11', '11 - Very Strong Sodicity', '#800080', 1, 't')
...
-- BEGIN
--     FOR f IN
--         SELECT mapset_id FROM spatial_metadata.layer WHERE mapset_id ILIKE 'ID%' AND stats_minimum < -100
--     LOOP
