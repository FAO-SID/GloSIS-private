-- project
-- UPDATE spatial_metadata.project SET project_name='Global Salt-Affected Soils' WHERE country_id='BD' AND project_id='GSAS';
UPDATE spatial_metadata.project SET project_name='Global Soil Nutrients Map' WHERE country_id='BD' AND project_id='GSNM';
UPDATE spatial_metadata.project SET project_name='Global Soil Organic Carbon Sequestration potential map' WHERE country_id='BD' AND project_id='GSOCSEQ';


-- dates
UPDATE spatial_metadata.mapset SET creation_date = '2020-12-16', publication_date = '2020-12-16', revision_date = '2020-12-16' WHERE country_id = 'BD' AND project_id = 'GSAS';
UPDATE spatial_metadata.mapset SET creation_date = '2024-12-24', publication_date = '2024-12-24', revision_date = '2024-12-24' WHERE country_id = 'BD' AND project_id = 'GSNM';
UPDATE spatial_metadata.mapset SET creation_date = '2021-05-11', publication_date = '2021-05-11', revision_date = '2021-05-11' WHERE country_id = 'BD' AND project_id = 'GSOCSEQ';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    ...
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'BD' AND project_id='GSAS';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'BD' AND project_id='GSNM';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'BD' AND project_id='GSOCSEQ';
...
WHERE m.mapset_id = l.mapset_id AND m.country_id = 'BD';


-- keywords
UPDATE spatial_metadata.mapset SET keyword_place = '{Bangladesh}'::text[] WHERE country_id = 'BD';
UPDATE spatial_metadata.mapset m SET keyword_theme = p.keyword_theme FROM spatial_metadata.property p WHERE country_id = 'BD' AND m.property_id = p.property_id;
...
-- time period
UPDATE spatial_metadata.mapset SET time_period_begin = '1970-01-13' WHERE country_id = 'BD';
UPDATE spatial_metadata.mapset SET time_period_end = '1988-01-13' WHERE country_id = 'BD';
...
INSERT INTO spatial_metadata.organisation (organisation_id, url, email, country, city, postal_code, delivery_point) VALUES
    ('Soil Resource Development Institute', 'https://srdi.govt.bd', 'sfri.org@gmail.com', 'Bangladesh', 'Dhaka', '1200', 'Krishi Khamar Sarak')
...
INSERT INTO spatial_metadata.proj_x_org_x_ind (country_id, project_id, tag, "role", "position", organisation_id, individual_id)
    SELECT DISTINCT country_id, project_id, 'contact', 'resourceProvider', 'Chief Scientific Officer', 'Soil Resource Development Institute', 'Farzana Shahrin' FROM spatial_metadata.project WHERE country_id = 'BD'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Chief Scientific Officer', 'Soil Resource Development Institute', 'Farzana Shahrin' FROM spatial_metadata.project WHERE country_id = 'BD'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Principal Scientific Officer', 'Soil Resource Development Institute', 'Manzurul Hoque' FROM spatial_metadata.project WHERE country_id = 'BD'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Chief Scientific Officer', 'Soil Resource Development Institute', 'Ruhul Islam' FROM spatial_metadata.project WHERE country_id = 'BD'
...
INSERT INTO spatial_metadata.url (mapset_id, protocol, url, url_name)
    ...
    --     UNION
    SELECT DISTINCT mapset_id, 'WWW:LINK-1.0-http--link', 'https://www.fao.org/global-soil-partnership/resources/highlights/detail/en/c/1471478/', 'Project webpage' FROM spatial_metadata.mapset WHERE project_id = 'GSOCSEQ' AND country_id = 'BD'
      UNION
    ...
    SELECT m.mapset_id, 'WWW:LINK-1.0-http--link', 'https://storage.googleapis.com/fao-gismgr-glosis-data/DATA/GLOSIS/MAPSET/'||l.mapset_id||'/GLOSIS.'||l.mapset_id||'.D-'||l.dimension_depth||'.D-'||l.dimension_stats||'.'||l.file_extension , 'Download '||l.dimension_depth||' '||l.dimension_stats
        ...
        WHERE m.country_id = 'BD'
          ...
          AND l.dimension_stats IS NOT NULL
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'https://data.apps.fao.org/map/wmts/wmts?service=WMTS&amp;request=GetCapabilities&amp;version=1.0.0&amp;workspace=GLOSIS', 'WMTS (FAO)' FROM spatial_metadata.mapset WHERE country_id = 'BD'
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'http://localhost:8082/?map=/etc/mapserver/'||layer_id||'.map&amp;SERVICE=WMS&amp;REQUEST=GetCapabilities', 'WMTS (Bangladesh SIS)' FROM spatial_metadata.layer WHERE layer_id LIKE 'BD-%'
