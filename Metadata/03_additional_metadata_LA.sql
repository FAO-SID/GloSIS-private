-- project
UPDATE spatial_metadata.project SET project_name='Global Salt-Affected Soils' WHERE country_id='LA' AND project_id='GSAS';
-- UPDATE spatial_metadata.project SET project_name='Global Soil Nutrients Map' WHERE country_id='LA' AND project_id='GSNM';
-- UPDATE spatial_metadata.project SET project_name='Global Soil Organic Carbon Sequestration potential map' WHERE country_id='LA' AND project_id='GSOCSEQ';


-- dates
UPDATE spatial_metadata.mapset SET creation_date = '2020-05-01', publication_date = '2020-05-01', revision_date = '2020-05-01' WHERE country_id = 'LA' AND project_id = 'GSAS';
-- UPDATE spatial_metadata.mapset SET creation_date = '2024-12-31', publication_date = '2024-12-31', revision_date = '2024-12-31' WHERE country_id = 'LA' AND project_id = 'GSNM';
-- UPDATE spatial_metadata.mapset SET creation_date = '2022-04-13', publication_date = '2022-04-13', revision_date = '2022-04-13' WHERE country_id = 'LA' AND project_id = 'GSOCSEQ';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    ...
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'LA' AND project_id='GSAS';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'LA' AND project_id='GSNM';
...
UPDATE spatial_metadata.mapset m SET title = t.title
    FROM (SELECT DISTINCT m.mapset_id, 
            ...
            LEFT JOIN spatial_metadata.country c ON c.country_id = m.country_id
            LEFT JOIN spatial_metadata.layer l ON l.mapset_id = m.mapset_id) t
    WHERE m.mapset_id = t.mapset_id AND m.country_id = 'LA' AND project_id='GSOCSEQ';
...
WHERE m.mapset_id = l.mapset_id AND m.country_id = 'LA';


-- keywords
UPDATE spatial_metadata.mapset SET keyword_place = '{Laos}'::text[] WHERE country_id = 'LA';
UPDATE spatial_metadata.mapset m SET keyword_theme = p.keyword_theme FROM spatial_metadata.property p WHERE country_id = 'LA' AND m.property_id = p.property_id;


-- licence
UPDATE spatial_metadata.mapset SET other_constraints = 'CC-BY-4.0' WHERE country_id = 'LA';


-- time period
UPDATE spatial_metadata.mapset SET time_period_begin = '2000-01-01' WHERE country_id = 'LA';
UPDATE spatial_metadata.mapset SET time_period_end = '2019-12-30' WHERE country_id = 'LA';
...
INSERT INTO spatial_metadata.organisation (organisation_id, url, email, country, city, postal_code, delivery_point) VALUES
    ('Ministry of Agriculture and Environment - Department of Land Administration and Management', NULL, NULL, 'Laos', 'Vientiane Capital', NULL, 'Khounboulom Street')
...
INSERT INTO spatial_metadata.proj_x_org_x_ind (country_id, project_id, tag, "role", "position", organisation_id, individual_id)
    SELECT DISTINCT country_id, project_id, 'contact', 'resourceProvider', 'GIS expert', 'Ministry of Agriculture and Environment - Department of Land Administration and Management', 'Sorlaty Sengxeu' FROM spatial_metadata.project WHERE country_id = 'LA'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'GIS expert', 'Ministry of Agriculture and Environment - Department of Land Administration and Management', 'Sorlaty Sengxeu' FROM spatial_metadata.project WHERE country_id = 'LA'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Director of Division', 'Ministry of Agriculture and Environment - Department of Land Administration and Management', 'Saysongkham Sayavong' FROM spatial_metadata.project WHERE country_id = 'LA'
            UNION
    SELECT DISTINCT country_id, project_id, 'pointOfContact', 'author', 'Deputy Director General', 'Ministry of Agriculture and Environment - Department of Land Administration and Management', 'Thatheva Saphangthong' FROM spatial_metadata.project WHERE country_id = 'LA'
...
INSERT INTO spatial_metadata.url (mapset_id, protocol, url, url_name)
    ...
    --     UNION
    SELECT DISTINCT mapset_id, 'WWW:LINK-1.0-http--link', 'https://www.fao.org/global-soil-partnership/resources/highlights/detail/en/c/1471478/', 'Project webpage' FROM spatial_metadata.mapset WHERE project_id = 'GSOCSEQ' AND country_id = 'LA'
      UNION
    ...
    SELECT m.mapset_id, 'WWW:LINK-1.0-http--link', 'https://storage.googleapis.com/fao-gismgr-glosis-data/DATA/GLOSIS/MAPSET/'||l.mapset_id||'/GLOSIS.'||l.mapset_id||'.D-'||l.dimension_depth||'.D-'||l.dimension_stats||'.'||l.file_extension , 'Download '||l.dimension_depth||' '||l.dimension_stats
        ...
        WHERE m.country_id = 'LA'
          ...
          AND l.dimension_stats IS NOT NULL
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'https://data.apps.fao.org/map/wmts/wmts?service=WMTS&amp;request=GetCapabilities&amp;version=1.0.0&amp;workspace=GLOSIS', 'WMTS (FAO)' FROM spatial_metadata.mapset WHERE country_id = 'LA'
      UNION
    SELECT mapset_id, 'OGC:WMTS', 'http://localhost:8082/?map=/etc/mapserver/'||layer_id||'.map&amp;SERVICE=WMS&amp;REQUEST=GetCapabilities', 'WMTS (Laos SIS)' FROM spatial_metadata.layer WHERE layer_id LIKE 'LA-%'
...
('LA-GSAS-SALT-2020', -9999, '-9999', 'No Data', '#000000', 0, 't'),
('LA-GSAS-SALT-2020', 1, '1', '1 - Extreme Salinity', '#ff0000', 1, 't'),
('LA-GSAS-SALT-2020', 2, '2', '2 - Moderate Salinity', '#f5deb3', 1, 't'),
('LA-GSAS-SALT-2020', 3, '3', '3 - Moderate Sodicity', '#ee82ee', 1, 't'),
('LA-GSAS-SALT-2020', 4, '4', '4 - None', '#ffffff', 1, 't'),
('LA-GSAS-SALT-2020', 5, '5', '5 - Saline Sodic', '#00ffff', 1, 't'),
('LA-GSAS-SALT-2020', 6, '6', '6 - Slight Salinity', '#90ee90', 1, 't'),
('LA-GSAS-SALT-2020', 7, '7', '7 - Slight Sodicity', '#add8e6', 1, 't'),
('LA-GSAS-SALT-2020', 8, '8', '8 - Strong Salinity', '#f08080', 1, 't'),
('LA-GSAS-SALT-2020', 9, '9', '9 - Strong Sodicity', '#da70d6', 1, 't'),
('LA-GSAS-SALT-2020', 10, '10', '10 - Very Strong Salinity', '#f84040', 1, 't'),
('LA-GSAS-SALT-2020', 11, '11', '11 - Very Strong Sodicity', '#800080', 1, 't')
